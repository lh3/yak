...
int main_isec(int argc, char *argv[])
{
	...
	if (argc - o.ind < 2) {
		fprintf(stderr, "Usage: yak isec [options] <in1.yak> <in2.yak> [in3.yak ...]\n");
		fprintf(stderr, "Options:\n");
		fprintf(stderr, "  -t INT     number of worker threads [%d]\n", n_thread);
		fprintf(stderr, "  -o FILE    output dump file [%s]\n", fn_out);
		return 1;
	}
	h0 = yak_ch_restore(argv[o.ind]);
	if (h0 == 0) {
		fprintf(stderr, "ERROR: failed to load hash table from '%s'\n", argv[o.ind]);
		return 1;
	}
	for (i = o.ind + 1; i < argc; ++i) {
		h1 = yak_ch_restore(argv[i]);
		if (h1 == 0) {
			fprintf(stderr, "ERROR: failed to load hash table from '%s'\n", argv[i]);
			yak_ch_destroy(h0);
			return 1;
		}
		yak_ch_isec(h0, h1, n_thread);
		yak_ch_destroy(h1);
	}
	yak_ch_tighten(h0);
	yak_ch_dump(h0, fn_out);
...
int main(int argc, char *argv[])
{
	...
	if (argc == 1) {
		...
		fprintf(stderr, "  isec      intersect k-mer sets\n");
	...
	else if (strcmp(argv[1], "isec") == 0) ret = main_isec(argc-1, argv+1);
